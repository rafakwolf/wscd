...
begin
  try
    ...
    MsgAviso('Data inicial inv�lida.');
    medtDataI.SetFocus;
    Exit;
  end;
...
begin
  ...
  begin
    MsgAviso('N�o existem dados para gerar relat�rio.');
    Exit;
  end;
  ...
  mensagem.Add('********************RELAT�RIO DE ERROS********************');
  ...
  try
    ...
    sdlgLogErro.FileName := 'Relat�rio de erros.txt';
    frmAguarde.Fecha;
    if sdlgLogErro.Execute then
    begin

      mensagem.SaveToFile(sdlgLogErro.FileName);
      if MsgSN('Relat�rio gerado com sucesso, deseja visualiz�-lo?') then
...
begin
  inherited;
  if not MsgSN('Deseja excluir o hist�rico de erros do sistema?') then
    Exit;
