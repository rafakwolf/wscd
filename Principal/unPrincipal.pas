...
//      begin
//        if Application.MessageBox('A chave de libera��o informada est� incorreta ' +
//          'ou ainda n�o foi informada, os dados da empresa tamb�m podem ter sido alterados.' + #13 +
...
//    tvExpirouPrazo:
//      begin
//        if Application.MessageBox('A chave de libera��o est� expirada.' + #13 +
...
//          'Deseja informar uma nova chave de libera��o agora?', 'Chave expirada',
//          MB_YESNO or MB_ICONWARNING) = ID_YES then
//          SimNao := True
//        else
//          SimNao := False;
...
//        ChamaForm('TfrmRegistro', 'Registro - Valida��o do Serial', Self);
//      end;
//  end;
//
...
//        ChamaForm('TfrmRenovaChave', 'Renova��o da chave de libera��o', Self)
//      else
//        ChamaForm('TfrmRegistro', 'Registro - Valida��o do Serial', Self);
//    end
...
begin
  ChamaForm('TfrmConfigGlobal', 'Configura��es globais', Self);
end;

procedure TfrmPrincipal.actConfiguracaoLocalExecute(Sender: TObject);
begin
  ChamaForm('TfrmConfiguracao', 'Configura��es locais', Self);
end;
...
begin
  if PrinterExists then
    ...
    MsgCuidado(MSG_IMPRESSORA, 'Impressora n�o Instalada');
end;
...
begin
  ChamaForm('TfrmUsuarioItemMenu', 'Usu�rios', Self);
end;
...
begin
  ChamaForm('TfrmCalendario', 'Calend�rio', Self);
end;
...
begin
  ChamaForm('TfrmDuplicatas', 'Cadastro e impress�o de duplicatas', Self);
end;

procedure TfrmPrincipal.actBackupExecute(Sender: TObject);
begin
  ChamaForm('TfrmBackup', 'Fazer uma c�pia dos dados', self);
end;
...
begin
  ChamaForm('TfrmOrcamentos', 'Or�amentos', Self);
end;
...
begin
  ChamaForm('TfrmExclusaoProduto', 'Exclus�o de produtos', Self);
end;

procedure TfrmPrincipal.actListaPrecoExecute(Sender: TObject);
begin
  ChamaForm('TfrmListagemPrecos', 'Lista de pre�os', Self);
end;
...
begin
  ChamaForm('TfrmPerfilPermissao', 'Perfis e permiss�es de usu�rios', Self);
end;
...
begin
  if copiando then
  begin
    if MsgSN('Existe uma copia autom�tica em andamento, deseja finalizar mesmo assim?') then
    begin
      CanClose := True;
    end
    else
  ...
  begin
    case Application.MessageBox('Deseja efetuar uma c�pia de seguran�a agora?', 'Finalizando o sistema',
      MB_YESNOCANCEL or MB_ICONQUESTION) of
      ID_YES:
        begin
          CanClose := False;
          ChamaForm('TfrmBackup', 'C�pia de seguran�a', Self);
        end;
      ID_NO:
        begin
          Application.Terminate;
...
//      'O sistema ser� finalizado.', 'Data inv�lida');
//    SistemaOk := False;
//    Application.Terminate;
//  end;
end;
...
begin
  ...
  sbPrincipal.Panels[0].Text := 'Usu�rio: ' + AnsiUpperCase(Usuario);
...
begin
  ...
  try
    ...
    { apos as atualiza��es vai mostrar "o que h� de novo" no sistema }
...
begin
  ...
  begin
    MsgAviso('','Esta altera��o s� ter� efeito da pr�xima vez que o sistema for iniciado.');
  end;
end;

procedure TfrmPrincipal.actNotaPromissoriaExecute(Sender: TObject);
begin
  ChamaForm('TfrmPromissoria', 'Promiss�ria avulsa', Self);
end;

procedure TfrmPrincipal.actInfoSistemaExecute(Sender: TObject);
begin
  ChamaForm('TfrmInformacaoSistema', 'Informa��es sobre o sistema', Self);
end;
...
begin
  ChamaForm('TfrmConfigNota', 'Configur���o da nota', Self);
end;
...
//  begin
//    MsgCuidado('Verifique sua conex�o com a internet.');
//    Exit;
//  end;
//
//  if Atualizando then
//  begin
//    MsgAviso('Atualiza��o j� est� em andamento.');
//    Exit;
//  end;
//
...
//    begin
//      if MsgSN('J� existe uma atualiza��o baixada hoje! Deseja execut�-la?') then
//      begin
...
begin
  ...
  begin
    MsgAviso('Nova vers�o do sistema encontrada.' + #13#10 +
      'A atualiza��o autom�ticamente iniciar� agora.', 'Nova vers�o');
    ...
    begin
      ...
      begin
        MsgErro('Houveram problemas durante a atualiza��o, entre em contado com suporte.');
        Result := False;
        Application.Terminate;
      end
      else
      begin
        MsgAviso('Atualiza��o efetuada com sucesso. Vers�o ' + Sistema.VersaoApp + '.');
        Result := True;
  ...
  begin
    MsgCuidado('A vers�o da base de dados � mais recente que a vers�o do execut�vel.' + #13 +
...
begin
  ChamaForm('TfrmPromocao', 'Promo��es', Self);
end;
...
//  else
//    msg := msg + 'Erro N�O gravado pelo sistema.';
...
begin
  ChamaForm('TfrmForceAtualizacao', 'Atualiza��o', Self);
end;
...
begin
  try
    ...
    if X < BarWidth then
      sbPrincipal.Hint := 'Usu�rio logado'
    else
...
begin
  if Configuracao.InfoAvisos then
    ChamaForm('TfrmInfoAvisos', 'Informa��oes e avisos', Self);
end;

procedure TfrmPrincipal.actInfoAvisosExecute(Sender: TObject);
begin
  ChamaForm('TfrminfoAvisos', 'Informa��es e avisos', Self);
end;
...
begin
  ...
  begin
    if Application.MessageBox(PChar('Primeiro acesso ao sistema.' + #13#10 +
      'O sistema utiliza a data de acesso para validar sua chave de libera��o.' + #13#10 +
      'A data atual do seu computador � ' + FormatDateTime('dd/mm/yyyy', DataAtual) + ', continua?'),
    ...
    begin
      Application.MessageBox('Por favor, corrija a data do computador e tente novamente.' + #13#10 +
        'O sistema ser� finalizado.', 'Data inv�lida', MB_OK or MB_ICONINFORMATION);
      Application.Terminate;
      Abort;
    end;
  end
  ...
  begin
    ...
    if (Dias > 3) then
    begin
      if Application.MessageBox(PChar('A �ltima abertura do sistema foi registrada com a data ' +
        FormatDateTime('dd/mm/yyyy', DataUltimoAcesso) + '.' + #13#10 +
        'O registro de uma data incorreta pode invalidar a chave de libera��o deste sistema' + #13#10 +
        'e n�o ser� poss�vel desfazer esta opera��o sem uma nova chave.' + #13#10 +
        'Tem certeza que deseja registrar a data atual ' + FormatDateTime('dd/mm/yyyy', DataAtual) +
        ' como sendo a nova data de acesso?'), 'Viola��o de data',
      ...
      if (Dias < 0) then
      begin
        Application.MessageBox(PChar('A data do �ltimo acesso ao sistema foi ' +
          FormatDateTime('dd/mm/yyyy', DataUltimoAcesso) + '.' + #13 +
          'N�o � poss�vel abrir o sistema com a data atual sendo inferior � do �ltimo acesso.' + #13 +
...
begin
  ChamaForm('TfrmRepararIndice', 'Reparar �ndices', Self);
end;
