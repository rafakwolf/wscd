inherited frmReceberMan: TfrmReceberMan
  Left = 213
  Top = 153
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'Recebimento'
  ClientHeight = 417
  ClientWidth = 481
  OnShow = FormShow
  ExplicitWidth = 487
  ExplicitHeight = 446
  PixelsPerInch = 96
  TextHeight = 13
  object BevelLinha: TBevel
    Left = 6
    Top = 200
    Width = 467
    Height = 2
  end
  object lblBandaMagnetica: TLabel
    Left = 8
    Top = 292
    Width = 99
    Height = 13
    Caption = 'Banda magn'#233'tica'
  end
  object bvlLinha2: TBevel
    Left = 8
    Top = 288
    Width = 465
    Height = 2
  end
  object lblEmissao: TLabel
    Left = 8
    Top = 372
    Width = 47
    Height = 13
    Caption = 'Emiss'#227'o'
  end
  object lblVenc: TLabel
    Left = 120
    Top = 372
    Width = 56
    Height = 13
    Caption = 'Bom para'
  end
  object dbValor: TLabeledEdit
    Left = 8
    Top = 260
    Width = 161
    Height = 21
    EditLabel.Width = 88
    EditLabel.Height = 13
    EditLabel.Caption = 'Valor a receber'
    ReadOnly = True
    TabOrder = 2
  end
  object btnOk: TBitBtn
    Left = 304
    Top = 384
    Width = 81
    Height = 25
    Hint = 'Efetuar recebimento'
    Caption = '&Ok'
    Glyph.Data = {
      
      00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    NumGlyphs = 2
    TabOrder = 11
    OnClick = btnOkClick
  end
  object btnCancel: TBitBtn
    Left = 392
    Top = 384
    Width = 81
    Height = 25
    Hint = 'Cancelar recebimento'
    Caption = '&Cancelar'
    Glyph.Data = {
      
      00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    NumGlyphs = 2
    TabOrder = 12
    OnClick = btnCancelClick
  end
  object rgReceb: TRadioGroup
    Left = 8
    Top = 208
    Width = 465
    Height = 33
    Caption = ' Recebimento '
    Columns = 2
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Verdana'
    Font.Style = []
    ItemIndex = 0
    Items.Strings = (
      'Dinheiro'
      'Cheque')
    ParentFont = False
    TabOrder = 1
    OnClick = rgRecebClick
  end
  object dbgrdContas: TDBGrid
    Left = 8
    Top = 8
    Width = 465
    Height = 185
    Color = clBtnFace
    DataSource = dsContasReceber
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    TitleFont.Charset = ANSI_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Verdana'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'VALOR'
        Title.Caption = 'Valor'
        Title.Font.Charset = ANSI_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -12
        Title.Font.Name = 'Arial'
        Title.Font.Style = []
        Width = 91
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'JURO'
        Title.Caption = 'Juro (%)'
        Title.Font.Charset = ANSI_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -12
        Title.Font.Name = 'Arial'
        Title.Font.Style = []
        Width = 59
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ATRASO'
        Title.Caption = 'Atraso (dias)'
        Title.Font.Charset = ANSI_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -12
        Title.Font.Name = 'Arial'
        Title.Font.Style = []
        Width = 89
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'VALORJURO'
        Title.Caption = 'Valor Juro'
        Title.Font.Charset = ANSI_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -12
        Title.Font.Name = 'Arial'
        Title.Font.Style = []
        Width = 84
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'TOTAL'
        Title.Caption = 'Valor atual'
        Title.Font.Charset = ANSI_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -12
        Title.Font.Name = 'Arial'
        Title.Font.Style = []
        Width = 95
        Visible = True
      end>
  end
  object edtAgencia: TLabeledEdit
    Left = 8
    Top = 348
    Width = 105
    Height = 21
    EditLabel.Width = 45
    EditLabel.Height = 13
    EditLabel.Caption = 'Ag'#234'ncia'
    TabOrder = 5
  end
  object dbeBanco: TDBEdit
    Left = 248
    Top = 308
    Width = 203
    Height = 21
    Hint = 'Pesquisar < F2 >'
    Color = clBtnFace
    DataField = 'BANCO'
    DataSource = dsSelecao
    ReadOnly = True
    TabOrder = 4
  end
  object edtConta: TLabeledEdit
    Left = 120
    Top = 348
    Width = 105
    Height = 21
    EditLabel.Width = 34
    EditLabel.Height = 13
    EditLabel.Caption = 'Conta'
    TabOrder = 6
  end
  object edtNumeroCheque: TLabeledEdit
    Left = 232
    Top = 348
    Width = 105
    Height = 21
    EditLabel.Width = 45
    EditLabel.Height = 13
    EditLabel.Caption = 'N'#250'mero'
    TabOrder = 7
  end
  object edtValor: TLabeledEdit
    Left = 344
    Top = 348
    Width = 129
    Height = 21
    EditLabel.Width = 92
    EditLabel.Height = 13
    EditLabel.Caption = 'Valor do cheque'
    TabOrder = 8
    OnEnter = edtValorEnter
  end
  object medtBandaMagnetica: TMaskEdit
    Left = 8
    Top = 308
    Width = 233
    Height = 21
    TabOrder = 3
    Text = ''
    OnExit = medtBandaMagneticaExit
  end
  object medtVencimento: TMaskEdit
    Left = 120
    Top = 388
    Width = 105
    Height = 21
    EditMask = '!99/99/9999;1;_'
    MaxLength = 10
    TabOrder = 10
    Text = '  /  /    '
  end
  object medtEmissao: TMaskEdit
    Left = 8
    Top = 388
    Width = 105
    Height = 21
    EditMask = '!99/99/9999;1;_'
    MaxLength = 10
    TabOrder = 9
    Text = '  /  /    '
  end
  object dsContasReceber: TDataSource
    AutoEdit = False
    DataSet = cdsContaReceber
    Left = 352
    Top = 72
  end
  object sqldContaReceber: TSQLDataSet
    CommandText = 
      'SELECT'#13#10'  CR.CODIGO,'#13#10'  CR.DATA,'#13#10'  CR.VENCIMENTO,'#13#10'  CR.CLIENTE' +
      ','#13#10'  CLI.NOME AS NOMECLIENTE,'#13#10'  CR.IDCONTA,'#13#10'  CC.NOME,'#13#10'  CR.D' +
      'ESCRICAO,'#13#10'  CR.DOCUMENTO,'#13#10'  CR.VALOR,'#13#10'  CR.JURO,'#13#10'  CR.RECEBE' +
      'R,'#13#10'  CR.RECDA,'#13#10'  CR.DATARECTO,'#13#10'  CR.ORIGEM,'#13#10'  CR.VENDA,'#13#10'  C' +
      'R.CAPITALRECDO,'#13#10'  CR.JURORECDO,'#13#10'  CR.DESCTO,'#13#10'  CR.OBS,'#13#10'  CR.' +
      'ATRASO,'#13#10'  CR.VALORJURO,'#13#10'  CR.TOTAL,'#13#10'  CR.TOTALRECDO'#13#10'FROM CON' +
      'TASRECEBER CR'#13#10'LEFT JOIN CLIENTES CLI ON (CR.CLIENTE = CLI.CODCL' +
      'IENTE)'#13#10'LEFT JOIN CAIXAS CC ON (CC.CODIGO = CR.IDCONTA)'#13#10'WHERE (' +
      'CR.RECEBER = '#39'S'#39')'
    MaxBlobSize = 1
    Params = <>
    Left = 256
    Top = 72
    object sqldContaReceberCODIGO: TIntegerField
      FieldName = 'CODIGO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object sqldContaReceberDATA: TDateField
      FieldName = 'DATA'
    end
    object sqldContaReceberVENCIMENTO: TDateField
      FieldName = 'VENCIMENTO'
    end
    object sqldContaReceberCLIENTE: TIntegerField
      FieldName = 'CLIENTE'
    end
    object sqldContaReceberNOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      ProviderFlags = []
      Size = 80
    end
    object sqldContaReceberDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 80
    end
    object sqldContaReceberDOCUMENTO: TStringField
      FieldName = 'DOCUMENTO'
    end
    object sqldContaReceberVALOR: TFMTBCDField
      FieldName = 'VALOR'
      Precision = 15
    end
    object sqldContaReceberJURO: TFMTBCDField
      FieldName = 'JURO'
      Precision = 15
    end
    object sqldContaReceberRECEBER: TStringField
      FieldName = 'RECEBER'
      FixedChar = True
      Size = 1
    end
    object sqldContaReceberRECDA: TStringField
      FieldName = 'RECDA'
      FixedChar = True
      Size = 1
    end
    object sqldContaReceberDATARECTO: TDateField
      FieldName = 'DATARECTO'
    end
    object sqldContaReceberORIGEM: TIntegerField
      FieldName = 'ORIGEM'
    end
    object sqldContaReceberVENDA: TIntegerField
      FieldName = 'VENDA'
    end
    object sqldContaReceberCAPITALRECDO: TFMTBCDField
      FieldName = 'CAPITALRECDO'
      Precision = 15
    end
    object sqldContaReceberJURORECDO: TFMTBCDField
      FieldName = 'JURORECDO'
      Precision = 15
    end
    object sqldContaReceberDESCTO: TFMTBCDField
      FieldName = 'DESCTO'
      Precision = 15
    end
    object sqldContaReceberOBS: TMemoField
      FieldName = 'OBS'
      BlobType = ftMemo
      Size = 1
    end
    object sqldContaReceberATRASO: TIntegerField
      FieldName = 'ATRASO'
    end
    object sqldContaReceberIDCONTA: TIntegerField
      FieldName = 'IDCONTA'
    end
    object sqldContaReceberNOME: TStringField
      FieldName = 'NOME'
      Size = 80
    end
    object sqldContaReceberVALORJURO: TSingleField
      FieldName = 'VALORJURO'
    end
    object sqldContaReceberTOTAL: TSingleField
      FieldName = 'TOTAL'
    end
    object sqldContaReceberTOTALRECDO: TSingleField
      FieldName = 'TOTALRECDO'
    end
  end
  object dspContaReceber: TDataSetProvider
    DataSet = sqldContaReceber
    UpdateMode = upWhereKeyOnly
    Left = 288
    Top = 72
  end
  object cdsContaReceber: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspContaReceber'
    AfterInsert = cdsContaReceberAfterInsert
    Left = 320
    Top = 72
    object cdsContaReceberCODIGO: TIntegerField
      FieldName = 'CODIGO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object cdsContaReceberDATA: TDateField
      FieldName = 'DATA'
      DisplayFormat = 'dd/mm/yyyy'
    end
    object cdsContaReceberVENCIMENTO: TDateField
      FieldName = 'VENCIMENTO'
      DisplayFormat = 'dd/mm/yyyy'
    end
    object cdsContaReceberCLIENTE: TIntegerField
      FieldName = 'CLIENTE'
    end
    object cdsContaReceberNOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      ProviderFlags = []
      Size = 80
    end
    object cdsContaReceberDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 80
    end
    object cdsContaReceberDOCUMENTO: TStringField
      FieldName = 'DOCUMENTO'
    end
    object cdsContaReceberVALOR: TFMTBCDField
      FieldName = 'VALOR'
      DisplayFormat = '#,##0.00'
      Precision = 15
    end
    object cdsContaReceberJURO: TFMTBCDField
      FieldName = 'JURO'
      DisplayFormat = '#,##0.00'
      Precision = 15
    end
    object cdsContaReceberRECEBER: TStringField
      FieldName = 'RECEBER'
      FixedChar = True
      Size = 1
    end
    object cdsContaReceberRECDA: TStringField
      FieldName = 'RECDA'
      FixedChar = True
      Size = 1
    end
    object cdsContaReceberDATARECTO: TDateField
      FieldName = 'DATARECTO'
      DisplayFormat = 'dd/mm/yyyy'
    end
    object cdsContaReceberORIGEM: TIntegerField
      FieldName = 'ORIGEM'
    end
    object cdsContaReceberVENDA: TIntegerField
      FieldName = 'VENDA'
    end
    object cdsContaReceberCAPITALRECDO: TFMTBCDField
      FieldName = 'CAPITALRECDO'
      DisplayFormat = '#,##0.00'
      Precision = 15
    end
    object cdsContaReceberJURORECDO: TFMTBCDField
      FieldName = 'JURORECDO'
      DisplayFormat = '#,##0.00'
      Precision = 15
    end
    object cdsContaReceberDESCTO: TFMTBCDField
      FieldName = 'DESCTO'
      DisplayFormat = '#,##0.00'
      Precision = 15
    end
    object cdsContaReceberOBS: TMemoField
      FieldName = 'OBS'
      BlobType = ftMemo
      Size = 1
    end
    object cdsContaReceberATRASO: TIntegerField
      FieldName = 'ATRASO'
    end
    object cdsContaReceberIDCONTA: TIntegerField
      FieldName = 'IDCONTA'
    end
    object cdsContaReceberNOME: TStringField
      FieldName = 'NOME'
      Size = 80
    end
    object cdsContaReceberVALORJURO: TSingleField
      FieldName = 'VALORJURO'
    end
    object cdsContaReceberTOTAL: TSingleField
      FieldName = 'TOTAL'
    end
    object cdsContaReceberTOTALRECDO: TSingleField
      FieldName = 'TOTALRECDO'
    end
  end
  object sqldSelecao: TSQLDataSet
    CommandText = 
      'select'#13#10'  cast(null as integer) idbanco,'#13#10'  cast(null as varchar' +
      '(80)) banco'#13#10'from rdb$database'
    MaxBlobSize = -1
    Params = <>
    Left = 256
    Top = 104
    object sqldSelecaoIDBANCO: TIntegerField
      FieldName = 'IDBANCO'
    end
    object sqldSelecaoBANCO: TStringField
      FieldName = 'BANCO'
      Size = 80
    end
  end
  object dspSelecao: TDataSetProvider
    DataSet = sqldSelecao
    Left = 288
    Top = 104
  end
  object cdsSelecao: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspSelecao'
    Left = 320
    Top = 104
    object cdsSelecaoIDBANCO: TIntegerField
      FieldName = 'IDBANCO'
      OnValidate = cdsSelecaoIDBANCOValidate
    end
    object cdsSelecaoBANCO: TStringField
      FieldName = 'BANCO'
      Size = 80
    end
  end
  object dsSelecao: TDataSource
    DataSet = cdsSelecao
    Left = 353
    Top = 104
  end
end
