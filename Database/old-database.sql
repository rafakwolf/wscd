...
'Não é permitida a exclusão deste perfil';
CREATE EXCEPTION EXC_EXCLUSAO_REGISTRO
'Não é permitido excluir este registro pois ele é padrão.';
CREATE EXCEPTION EXC_INCLUSAO_REGISTRO
'Não é permitido incluir registros nesta tabela.';
/******************** TRIGGERS ********************/
...
BEGIN
  /* Se houver log aberto para a conexão corrente então vincula a operação
     a esse log, do contrário a variável será NULL e o log não vinculado. */
...
BEGIN
  ...
  select max(codigo)+1 from caixas into :IDCONTACAIXA;
   ...
   /* se não tiver conta configurada de cheque então cadastra */
  ...
  /* configurações padronizadas na primeira abertura do sistema */
...
BEGIN
  /* busca o documento e se a conta já foi paga */
   SELECT
     cr.Documento,
     cr.Paga
   FROM Contaspagar cr
  ...
  /* caso a conta já tenha sido paga, elimina ela do caixa também */
  IF (:VPAGA = 'S') THEN
    DELETE FROM Caixa c
...
BEGIN
  ...
  /* se o conta já foi recebida, exclui o lançamento no caixa também */
  IF (:VRECDA = 'S') THEN
    DELETE FROM Caixa c
     WHERE c.Documento = :VDOC;
END^
...
BEGIN
  ...
  /* coloca a venda como NÃO concluída */
...
BEGIN
  ...
  /* se o restoatual for zero, entende que ainda não foi feito nenhum pagamento */
  ...
  /* só conclui a compra caso não tenha mais resto,ou o valor a pagar for igual
     ao total da compra */
...
BEGIN
  ...
  /* se o restoatual for zero, entende que ainda não foi feito nenhum recebimento */
  IF (:RESTOATUAL > 0) THEN
  ...
  /* só conclui a venda caso não tenha mais resto ou o valor a receber for igual
     ao total da venda */
