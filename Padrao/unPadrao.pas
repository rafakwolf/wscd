  ...
  try
    ...
    begin
      if ((TClientDataSet(dsPadrao.DataSet).ChangeCount <> 0) or (TClientDataSet(dsPadrao.DataSet).Modified)) then
        case
          Application.MessageBox(PChar('Existem altera��es pendentes no processo "'
          +
          Caption + '".' + #13#10 + 'Deseja gravar as altera��es?'),
            'Confirma��o',
...
begin
  ...
  Application.MessageBox(PChar(S), 'Exclus�o n�o permitida', MB_OK or
    MB_ICONWARNING);
  Action := raCancel;
end;
...
//      'Exclus�o de registro', MB_ICONQUESTION or MB_YESNO or MB_DEFBUTTON2) =  IDYES then
//    begin
...
begin
  if ((TableName = '') or
    ...
    raise
      Exception.Create('Erro ao executar pesquisa, verifique os par�metros.');
...
begin
  ...
  begin
    if dsPadrao.DataSet.IsEmpty then
    begin
      if MessageDlgCheck('N�o existe nenhum registro cadastrado nesta tela.' +
        #13 + 'Deseja cadastrar agora?', mtConfirmation, [mbYes, mbNo], -1, mrOk,
          True, True, 'N�o exibir novamente esta mensagem.', @ConfigMsgEmptyTable) = ID_YES then
        actInsert.Execute;
    end;
  end;
end;
